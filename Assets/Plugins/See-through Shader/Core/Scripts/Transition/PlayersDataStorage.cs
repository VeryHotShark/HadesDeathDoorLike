    ...
    public sealed class PlayersDataStorage
    {
        ...
        public void GeneratePlayerDataForAllPlayerPositions()
        {
            ...
            if (playerPosArray != null && playerPosArray.Length > 0 && _PlayerDataDict.Count <= 0 && arrayLength > 0)
            {
                ...
                {
                    _PlayerDataDict.Add((int)playerPosArray[i][3], new PlayerData((int)playerPosArray[i][3], i));
                }
        ...
        {
            _PlayerDataDict.Add(instancedId, new PlayerData(instancedId, index));
            UpdatePlayersDatasShaderFloatArray();
        }
        ...
        public void AddOrUpdatePlayerData(PlayerData playerData)
        {
            if (playerData != null)
            {
                if (!_PlayerDataDict.ContainsKey(playerData.PlayerID))
                {
                    _PlayerDataDict.Add(playerData.PlayerID, playerData);
                }
                else
                {
                    _PlayerDataDict[playerData.PlayerID] = playerData;
                }
        ...
        public void RemovePlayerData(PlayerData playerData)
        {
            if (playerData != null)
            {
                RemovePlayerData(playerData.PlayerID);
            }
        }
        ...
        public void UpdatePlayersDatasShaderFloatArray()
        {
            ...
            for (int i = 0; i < arrayLength; i++)
            {
                if (_PlayerDataDict.TryGetValue((int)playerPosArray[i][3], out PlayerData playerData))
                {
